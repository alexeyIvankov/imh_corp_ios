...
final public class PopMenuManager: NSObject {
    ...
    public var popMenu: PopMenuViewController!
...
extension PopMenuManager {
    ...
    public func present(sourceView: AnyObject? = nil, on viewController: UIViewController? = nil, animated: Bool = true, inset:UIEdgeInsets? = nil, completion: (() -> Void)? = nil) {
        prepareViewController(sourceView: sourceView)
        
        if inset != nil{
            self.correctFramePopupFrom(inset: inset!)
        }
    ...
    private func correctFramePopupFrom(inset:UIEdgeInsets){
        var frame = self.popMenu.absoluteSourceFrame
        frame?.origin.x -= inset.right
        frame?.origin.x += inset.left
        frame?.origin.y += inset.top
        frame?.origin.y -= inset.bottom
        self.popMenu.absoluteSourceFrame = frame
    }
}
